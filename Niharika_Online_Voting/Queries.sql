...
--fetch states table where state_name is Andhra Pradesh 
select * from states where state_name='Andhra Pradesh';
--display state details table where state name is Karnataka
select * from state_info where state_name='Karnataka';
--find all district details of the district
select district_name from district_info;
--find the details of the constituency of the Tirupati
select * from constituency_info where constituency_name='Tirupati';
--find count of the voter
select count(*) from VOTER;
--find pincode detail of the 517001 of the  city
select * from CITY where zip_code=517001;
...
select * from PARTY;
--find leader of the TDP party
select * from PARTY_INFO where party_name='TDP';
...
create procedure vote_count_can(m in varchar)
is
a number(10):=0;
b number(10):=0;
x number(10):=0;
...
/
begin
vote_count_can('Girish');
END;
/

--create procedure total_voted_population in legislative
create or replace procedure total_pop_legis
is
a number(10):=0;
begin
